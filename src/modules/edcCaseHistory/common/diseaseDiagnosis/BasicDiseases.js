...
var Call_Basic_Diseases = Cordova.call_basic_diseases;
...
module.exports = React.createClass({
    ...
    render: function() {
        ...
        var data = this.props.basicDiseases;
        var basicDiseases = data.array && data.array.length > 0 ? data.array.slice() : [];
        if (data.text) {
            basicDiseases.push({
                name: '备注：' + data.text
            });
        }
        var length = basicDiseases.length;
        var header = <Header disease={data} type={type} />;
        var footer = length > 0 ? <Footer content={basicDiseases[0]} /> : <EmptyData />;
        return (
